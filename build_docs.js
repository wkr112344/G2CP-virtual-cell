...
const enChildren = [
  ...
  new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: "\u00B9Xinjiang Medical University, Urumqi 830011, China", size: 22 })] }),
  pc(""),
  new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 200 }, children: [new TextRun({ text: "*Correspondence: 2742391737@qq.com", size: 20, color: "666666" })] }),
  hr(CONTENT_W),
  h2("Abstract"),
  ...
  tbl(["Metric","This study","[1]","Evaluation set note"],
    [["SMD (cosine)","1.288","[1] 1.85","ChEMBL therapeutics (both)"],["ECFP4 Tanimoto SMD (validation)","1.613","[1] 1.61","ChEMBL therapeutics (both)"],["Mantel self-consistency \u03c1","0.852","[1] 0.43","Subramanian PCL [5] (both); different PCL sets"],["Cell lines","162","[1] 5","—"]]),
...
const cnChildren = [
  ...
  new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: "\u00B9新疆医科大学，乌鲁木齐 830011", size: 22, font: "SimSun" })] }),
  ...
  h2("5. 结论"),
  p("我们独立构建并扩展UniPert-G2CP架构至162细胞系、32,039化合物、全基因组输出，建成可部署的虚拟细胞预测平台。架构可成功扩展，但性能对CPI训练词表高度敏感，揭示的是领域迁移瓶颈而非单纯的架构局限。本文提供了留出评测指标、论文同口径复刻与生物学验证案例作为透明参考。平台可按合理请求免费用于学术研究。"),
  h2("数据与代码可用性"),
